...
public interface ModularTool {
    ...
    static boolean canMine(ItemStack itemStack, BlockState state) {
    ...
    static boolean isModuleInstalledAndEnabled(ModularToolComponentRecord record, Module module)  {
        return record.hasModule(module) && record.isModuleEnabled(module);
    }
    static Optional<TooltipData> getTooltipData(ItemStack stack) {
    ...
    static Pair<BlockPos, BlockPos> getMiningCorners(BlockPos pos, Direction direction, MiningSizeCategoryData data) {
    ...
    // Module interface
    static void addModule(ItemStack stack, Module module) {
    ...
    static void removeModule(ItemStack stack, Module module) {
        ModularToolComponentRecord componentRecord = stack.get(ModRegistration.ModComponents.MODULAR_TOOL_COMPONENT);
        assert componentRecord != null;
        componentRecord = componentRecord.removeModule(module);
        stack.set(ModRegistration.ModComponents.MODULAR_TOOL_COMPONENT, componentRecord);
    }
    static boolean wouldBreak(ItemStack stack, PlayerEntity player, int i) {
        ModularToolComponentRecord componentRecord = stack.get(ModRegistration.ModComponents.MODULAR_TOOL_COMPONENT);
        assert componentRecord != null;
    ...
    static void damage(ItemStack stack, PlayerEntity player, int i) {
        stack.damage(i, player, EquipmentSlot.MAINHAND);
    }
    static boolean isModuleInstalled(ItemStack stack, Module module) {
        ModularToolComponentRecord componentRecord = stack.get(ModRegistration.ModComponents.MODULAR_TOOL_COMPONENT);
        assert componentRecord != null;
